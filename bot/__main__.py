...
from bot import bot, app, dispatcher, updater, botStartTime, IGNORE_PENDING_REQUESTS, IS_VPS, PORT, alive, web, OWNER_ID, AUTHORIZED_CHATS, LOGGER, FEEDS, rss_session, Interval, nox
...
def stats(update, context):
    ...
    stats = f'<b>Bot Uptime:</b> <code>{currentTime}</code>\n' \
            f'<b>Disk Space:</b> <code>{total}</code>\n' \
...
def start(update, context):
    ...
    buttons.buildbutton("Repo", "https://www.github.com/hsj51/hishirobot")
...
<br><br>
<b>/{BotCommands.SearchCommand}</b> [query]: Search for torrents using qbittorrent search plugins
<br><br>
...
botcmds = [
        ...
        (f'{BotCommands.MirrorCommand}', 'Mirror supported URLs'),
...
def main():
    ...
    if FEEDS:
        rss_init()
        LOGGER.info("RSS feeds function initiated.")
        if rss_session is not None:
            rss_session.start()
            LOGGER.info("Using SESSION_STRING for RSS feeds.")
